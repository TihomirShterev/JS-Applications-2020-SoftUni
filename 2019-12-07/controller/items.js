...
export function postCreate(ctx) {
  const { name, dateTime, imageURL, description } = ctx.params;
  const creator = sessionStorage.getItem("user");
  console.log(creator);

  create({ name, dateTime, imageURL, description, creator, count: 0, usersWhoCounted: [] })
    .then(res => {
      // console.log(res);
      ctx.redirect("#/home");
    })
...
export function postEdit(ctx) {
  ...
  update(id, { name, dateTime, imageURL, description })
    .then(res => {
      // console.log(res);
